...
############################################
from seq2seq.adapters import AdapterController, Adapter 
from seq2seq.hypercomplex.layers import  PHMLinear
from seq2seq.hypercomplex.inits import phm_init, glorot_uniform, glorot_normal
...
class T5LayerFF(nn.Module):
    def __init__(self, config, adapter_config=None):
        ...
        #######################################################################
        self.train_lang_adapters = config.train_lang_adapters and adapter_config.add_adapter_in_feed_forward
        self.train_task_adapters = config.train_task_adapters and adapter_config.add_adapter_in_feed_forward
        if self.train_lang_adapters:
            lang_adapter_config = copy.deepcopy(adapter_config)
            lang_adapter_config.reduction_factor = adapter_config.lang_reduction_factor
            lang_adapter_config.tasks = adapter_config.langs
            self.layer_lang_hypernet = AdapterController(lang_adapter_config)
        if self.train_task_adapters:
            adapter_config.reduction_factor = adapter_config.task_reduction_factor
            self.layer_task_hypernet = AdapterController(adapter_config)
        #######################################################################
        self.layer_norm = T5LayerNorm(config.d_model, eps=config.layer_norm_epsilon, adapter_config=adapter_config)
        self.dropout = nn.Dropout(config.dropout_rate)

    def forward(self, hidden_states, lang_block_adapters=None,
               task_block_adapters=None, lang=None, task=None):
        forwarded_states = self.layer_norm(hidden_states)
        forwarded_states = self.DenseReluDense(forwarded_states)
        #########################################################
        if self.train_lang_adapters:
            forwarded_states = self.layer_lang_hypernet(forwarded_states, lang)
        if self.train_task_adapters:
            forwarded_states = self.layer_task_hypernet(forwarded_states, task)
        #########################################################
...
class T5LayerSelfAttention(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False, adapter_config=None):
        ...
        self.train_lang_adapters = config.train_lang_adapters and adapter_config.add_adapter_in_self_attention
        if self.train_lang_adapters:
            lang_adapter_config = copy.deepcopy(adapter_config)
            lang_adapter_config.reduction_factor = adapter_config.lang_reduction_factor
            lang_adapter_config.tasks = adapter_config.langs
    ...
    def forward(
        ...
        ####################################################
        if self.train_lang_adapters:
           y = self.layer_lang_hypernet(y, lang)
        if self.train_task_adapters:
           y = self.layer_task_hypernet(y, task)
        ####################################################
...
class T5Block(nn.Module):
    ...
    def forward(
        ...
        self_attention_outputs = self.layer[0](
            ...
            use_cache=use_cache,
            output_attentions=output_attentions,
            lang_block_adapters=lang_block_adapters,
...
class T5Stack(T5PreTrainedModel):
    ...
    def per_layer_config(self, config, layer_id, adapter_config, is_decoder):
        """Sets the train_task_adapter and train_lang_adapters in the config, based on the 
        information given."""
        def is_valid_layer(layer_id, adapter_config, is_decoder,adapter_type):
            if adapter_type == "task":
               ...
               valid_layer_ids = adapter_config.lang_adapter_layers_encoder\
               if not is_decoder else adapter_config.lang_adapter_layers_decoder
        ...
        valid_lang_adapter_layer_id = is_valid_layer(layer_id, adapter_config, is_decoder, "lang")
        valid_task_adapter_layer_id = is_valid_layer(layer_id, adapter_config, is_decoder, "task")
        # Here we may want to remove adapters from the decoder part.
        add_lang_adapter = True if not is_decoder else adapter_config.lang_adapter_in_decoder
        add_task_adapter = True if not is_decoder else adapter_config.task_adapter_in_decoder
        config.train_lang_adapters = config.train_lang_adapters and\
                                     valid_lang_adapter_layer_id and\
                                     add_lang_adapter
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def init_W(self, in_feats_per_axis, out_feats_per_axis, W=None, W_left=None, W_right=None):
        if self.w_init == "phm":
            if self.factorized_phm:
                W_left.data = phm_init(phm_dim=self.phm_dim,
                              in_features=in_feats_per_axis,
                              out_features=self.phm_rank, 
                              transpose=False)
                W_right.data = phm_init(phm_dim=self.phm_dim,
                              in_features=self.phm_rank, 
                              out_features=out_feats_per_axis,
                              transpose=False)
            else:
                W.data = phm_init(phm_dim=self.phm_dim,
                              in_features=in_feats_per_axis,
                              out_features=out_feats_per_axis,
                              transpose=False)
