...
class BertSelfOutput(nn.Module):
    def __init__(self, config, adapter_config=None):
        ...
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        #######################################################################
        self.train_lang_adapters = config.train_lang_adapters
        self.train_task_adapters = config.train_task_adapters
        ...
        self.train_adapters = self.train_lang_adapters or self.train_task_adapters
        if self.train_adapters:
            self.dropout_at_end = adapter_config.dropout_at_end
        #######################################################################

    def forward(self, hidden_states, input_tensor, lang_block_adapters=None,
        task_block_adapters=None, lang=None, task=None):
        hidden_states = self.dense(hidden_states)
        if not self.train_adapters or (self.train_adapters and not self.dropout_at_end):
            hidden_states = self.dropout(hidden_states)
        ####################################################################
        if self.train_lang_adapters:
            hidden_states = self.layer_lang_hypernet(hidden_states, lang)
        if self.train_task_adapters:
            hidden_states = self.layer_task_hypernet(hidden_states, task)
        if self.train_adapters and self.dropout_at_end:
            hidden_states = self.dropout(hidden_states)
        ####################################################################
...
class BertAttention(nn.Module):
    def __init__(self, config, adapter_config=None):
        ...
        self.output = BertSelfOutput(config, adapter_config=adapter_config)
...
class BertLayer(nn.Module):
    def __init__(self, config, adapter_config=None):
        ...
        self.seq_len_dim = 1
        self.attention = BertAttention(config, adapter_config=adapter_config)
        self.is_decoder = config.is_decoder
        ...
        self.output = BertOutput(config, adapter_config=adapter_config)
    ...
    def feed_forward_chunk(self, attention_output, lang_block_adapters=None,
        task_block_adapters=None, lang=None, task=None): 
...
class BertEncoder(nn.Module):
    def __init__(self, config, adapter_config=None):
        ...
        self.config = config
        self.layer = nn.ModuleList([BertLayer(config, adapter_config=adapter_config)\
...
class BertModel(BertPreTrainedModel):
    ...
    def __init__(self, config, add_pooling_layer=True, adapter_config=None):
        super().__init__(config)
        self.config = config

        self.embeddings = BertEmbeddings(config)
        self.encoder = BertEncoder(config, adapter_config=adapter_config)
...
class BertForSequenceClassification(BertPreTrainedModel):
    ...
    def forward(
        ...
        """
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict
        # TODO: we consider task/lang embedding of None for now.
        outputs = self.bert(
            input_ids,
            attention_mask=attention_mask,
            token_type_ids=token_type_ids,
