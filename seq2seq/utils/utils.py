...
from seq2seq.third_party.models import BertForSequenceClassification
from seq2seq.third_party.models import MT5ForConditionalGeneration, T5ForConditionalGeneration, T5Config, MT5Config
import json 

from seq2seq.adapters import (AutoAdapterConfig, MetaLayersAdapterController, MetaAdapterController, \
    AdapterController, AdapterLayersOneHyperNetController, AdapterLayersHyperNet, AdapterHyperNet, Adapter,\
    TaskEmbeddingController, TaskHyperNet, LayerNormHyperNet, HyperComplexAdapter)
...
from seq2seq.third_party.models.bert import BertConfig, BertForSequenceClassification
...
def get_adapter_config(adapter_args, data_args, training_args, config):
    if adapter_args.train_task_adapters or adapter_args.train_lang_adapters or adapter_args.prefix_tuning or adapter_args.bitfit:
        adapter_config = AutoAdapterConfig.get(adapter_args.adapter_config_name)
        adapter_config.input_dim = config.d_model if not isinstance(config, BertConfig) else config.hidden_size

        if adapter_args.train_task_adapters:
            data_args.tasks = [data_args.task_name]
            adapter_config.tasks = data_args.tasks
            adapter_config.task_to_adapter = None 
            adapter_config.task_to_embeddings = None
...
# TODO: shall we also freeze task embedding controller for lang adapters?
def freeze_model_params(model, adapter_args):
    ...
    # adapter parameters like task embeddings and adapter controllers.
    if adapter_args.train_task_adapters or adapter_args.train_lang_adapters:
        freeze_params(model)
        for name, sub_module in model.named_modules():
            if isinstance(sub_module, (MetaAdapterController, AdapterController,\
                                       MetaLayersAdapterController, AdapterLayersOneHyperNetController,\
                                       AdapterLayersHyperNet, AdapterHyperNet, Adapter,\
                                       TaskEmbeddingController, TaskHyperNet, LayerNormHyperNet)):
    ...
    # Freezes lang adapters.
    if adapter_args.freeze_lang_adapters:
        for n, m in model.named_modules():
             if isinstance(m, (MetaAdapterController, AdapterController,\
                                       MetaLayersAdapterController, AdapterLayersOneHyperNetController,\
                                       AdapterLayersHyperNet, AdapterHyperNet, Adapter,\
                                       TaskEmbeddingController, TaskHyperNet, LayerNormHyperNet)):
                if "lang" in n:
    ...
    if adapter_args.bitfit: 
        ...
        if adapter_args.freeze_bitfit_lm_head:
           if isinstance(model, BertForSequenceClassification):
              for n, param in model.classifier.named_parameters():
                 if "bias" in n:
                    param.requires_grad = True
                 else:
                    param.requires_grad = False
...
def modify_model_after_init(model, training_args, adapter_args):
    ...
    if adapter_args.intrinsic_adapters:
        selected_params = get_adapter_params_names(model)
        if adapter_args.intrinsic_layer_norms:
            selected_params += get_layer_norm_params_names(model)
        if adapter_args.intrinsic_said:
            model = intrinsic_dimension_said(model, adapter_args.intrinsic_dim,\
                training_args.output_dir, selected_params, adapter_args.intrinsic_projection, "cpu")
        else:
            model = intrinsic_dimension(model, adapter_args.intrinsic_dim,\
                training_args.output_dir, selected_params, adapter_args.intrinsic_projection, "cpu")
