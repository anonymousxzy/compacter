...
#from .utils import get_multiplication_matrices
from .inits import phm_init, glorot_uniform, glorot_normal
...
def matvec_product(W: torch.Tensor, x: torch.Tensor,
    ...
    if clamp:
        eps = 1e-10
        y = torch.clamp(torch.clamp(y, min=eps), max=4-eps)
    return y
...
class PHMLinear(torch.nn.Module):
    def __init__(self, in_features: int, out_features: int,
                 ...
                 learn_phm: bool = True, clamp=False, normalize=False,
                 shared_phm_rule=False, factorized_phm=False, 
                 shared_W_phm=False, factorized_phm_rule=False,
                 phm_rank = 1, phm_init_range=0.01,
    ...
    def init_W(self):
        if self.w_init == "phm":
            if self.factorized_phm:
                self.W_left.data = phm_init(phm_dim=self.phm_dim,
                              in_features=self._in_feats_per_axis,
                              out_features=self.phm_rank,
                              transpose=False)
                self.W_right.data = phm_init(phm_dim=self.phm_dim,
                              in_features=self.phm_rank,
                              out_features=self._out_feats_per_axis,
                              transpose=False)
            else:
                self.W.data = phm_init(phm_dim=self.phm_dim,
                              in_features=self._in_feats_per_axis,
            ...
            if self.factorized_phm:
                ...
                for i in range(self.phm_dim):
                    self.W.data[i].normal_(mean=0, std=phm_init_range)
        else:
            raise ValueError
    ...
    def forward(self, x: torch.Tensor, phm_rule: Union[None, nn.ParameterList] = None) -> torch.Tensor:
        ...
        return matvec_product(W=W if self.factorized_phm else self.W, 
               x=x, bias=self.b, 
               phm_rule=phm_rule if self.factorized_phm_rule else self.phm_rule, 
               clamp=self.clamp, normalize=self.normalize, kronecker_prod=self.kronecker_prod)
